...
func (aq *AccountQuery) QueryEntry() *EntryQuery {
	query := (&EntryClient{config: aq.config}).Query()
...
func (aq *AccountQuery) QueryTransfer() *TransferQuery {
	query := (&TransferClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withTransfer: aq.withTransfer.Clone(),
...
func (aq *AccountQuery) WithEntry(opts ...func(*EntryQuery)) *AccountQuery {
	query := (&EntryClient{config: aq.config}).Query()
...
func (aq *AccountQuery) WithTransfer(opts ...func(*TransferQuery)) *AccountQuery {
	query := (&TransferClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withTransfer = query
	return aq
}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withEntry; query != nil {
		if err := aq.loadEntry(ctx, query, nodes,
			func(n *Account) { n.Edges.Entry = []*Entry{} },
			func(n *Account, e *Entry) { n.Edges.Entry = append(n.Edges.Entry, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withTransfer; query != nil {
		if err := aq.loadTransfer(ctx, query, nodes,
			func(n *Account) { n.Edges.Transfer = []*Transfer{} },
			func(n *Account, e *Transfer) { n.Edges.Transfer = append(n.Edges.Transfer, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadEntry(ctx context.Context, query *EntryQuery, nodes []*Account, init func(*Account), assign func(*Account, *Entry)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(entry.FieldAccountID)
	}
	query.Where(predicate.Entry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.EntryColumn), fks...))
	}))
...
func (aq *AccountQuery) loadTransfer(ctx context.Context, query *TransferQuery, nodes []*Account, init func(*Account), assign func(*Account, *Transfer)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Transfer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.TransferColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_transfer
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_transfer" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_transfer" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
