...
func (eq *EntryQuery) QueryOwner() *AccountQuery {
...
func (eq *EntryQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (eq *EntryQuery) WithOwner(opts ...func(*AccountQuery)) *EntryQuery {
	query := (&AccountClient{config: eq.config}).Query()
...
func (eq *EntryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Entry, error) {
	...
	if query := eq.withOwner; query != nil {
		if err := eq.loadOwner(ctx, query, nodes, nil,
			func(n *Entry, e *Account) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EntryQuery) loadOwner(ctx context.Context, query *AccountQuery, nodes []*Entry, init func(*Entry), assign func(*Entry, *Account)) error {
...
func (eq *EntryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(entry.Table, entry.Columns, sqlgraph.NewFieldSpec(entry.FieldID, field.TypeUUID))
	_spec.From = eq.sql
	...
	if fields := eq.ctx.Fields; len(fields) > 0 {
		...
		if eq.withOwner != nil {
			_spec.Node.AddColumnOnce(entry.FieldAccountID)
		}
	}
